...
export class StockModel extends AbstractModels {
    async purchaseProductList(orgId) {
        const { quantity, subtotal, unitCost, productId, id } = getTableColumns(this.table.purchaseItems);
        ...
        const result = await this.db
            ...
            .leftJoin(this.table.products, eq(this.table.products.productId, this.table.purchaseItems.productId));
        return result;
    }
    async purchaseList(orgId) {
        ...
        const result = await this.db
            ...
            .leftJoin(this.table.suppliers, eq(this.table.purchases.supplierId, this.table.suppliers.supId))
            .leftJoin(this.table.purchaseItems, eq(this.table.purchases.purchaseId, this.table.purchaseItems.purchaseId))
            .leftJoin(this.table.products, eq(this.table.products.productId, this.table.purchaseItems.productId));
        return result;
    }
    ...
    async salesList(orgId) {
        const { saleDate, customerName, saleId } = getTableColumns(this.table.sales);
        const { name: product_name } = getTableColumns(this.table.products);
        const { id: salesItemId, subtotal, unitPrice, quantity, } = getTableColumns(this.table.saleItems);
        const result = await this.db
            .select({
            saleId,
            salesItemId,
            ...
            .leftJoin(this.table.saleItems, eq(this.table.sales.saleId, this.table.saleItems.saleId))
            .leftJoin(this.table.purchaseItems, eq(this.table.purchaseItems.id, this.table.saleItems.productId))
            .leftJoin(this.table.products, eq(this.table.products.productId, this.table.purchaseItems.productId));
        return result;
    }
