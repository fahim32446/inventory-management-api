...
export class AuthService {
    ...
    signUp = async (c) => {
        ...
        return await db.transaction(async (tx) => {
            ...
            const user = await this.db_conn.createAdminUser({ name, email, password: hashedPassword, agency_name: agency_name }, org.orgId, defaultRole.roleId, tx);
    ...
    refreshToken = async (c) => {
        ...
        const accessToken = await sign({ userId: session.userId, exp: Math.floor(Date.now() / 1000) * 60 * 15 }, env.JWT_SECRET);
    ...
    resetPassword = async (c) => {
        ...
        return await db.transaction(async (tx) => {
            ...
            const otpRecord = await this.db_conn.findOTP(existingUser.users.userId, code, "FORGOT_PASSWORD", tx);
